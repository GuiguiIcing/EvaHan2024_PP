...
class Load:
    def __init__(self, args):
        ...
        self.read_file()
        temp = sorted(self.count.keys(), key=lambda each: self.count[each], reverse=True)
        kk = 0
        for each in temp:
            if self.count[each] < 10:
                m = sorted(each, key=lambda e: self.count[e], reverse=True)[0]
                self.func[each] = m
                kk += 1
        label_length = len(temp) - kk
        print(self.func, len(self.func), len(temp))
        # print('-------------------')
        # for each in temp:
        #     print(each, self.count[each])
        # print('-------------------')
        # import sys
        # sys.exit(1)
        # self.read_file_twice()
        self.labels = [key for key in self.labels if key not in self.func]
        ...
        print(self.labels_dic, len(self.labels_dic))
        # import sys
        # sys.exit(1)
    ...
    def read_file(self):  # 数据清洗
        ...
        self.llm_train_ls = [self.signal_tag(list(line))
                             for line in json.load(f)]
        f.close()
        f = open('dataset/dev.json', mode='r')
        self.llm_dev_ls = [self.signal_tag(list(line))
                           for line in json.load(f)]
    ...
    def punc_to_ids(self):
        ...
        for words, tags in self.data_ls:
            ...
            tags = torch.tensor([self.labels_dic[t] if (t := self.func.get(e)) else self.labels_dic[e]
                                 for e in tags[:length - 2]])
